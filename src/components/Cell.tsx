...
function CellComponent({ cell, row, col }: CellProps) {
  ...
  if (cell.variant === 'empty') {
    return (
      <div data-row={row} data-col={col} data-variant="empty" className="aspect-square rounded-[3px] bg-white/[0.04]" />
    )
  }
  ...
  if (cell.variant === 'clearing') {
    return (
      ...
      >
        ...
        />
        <div data-clear-flash className="absolute inset-0 rounded-[3px] bg-white opacity-0 will-change-transform" />
        {particles.map((p, i) => (
          <span
            key={i}
            data-clear-particle
